...
declare variable $dynamicPossFilters := <fields>
    <field
        name="modalType">
        {
            for $x in request:get-parameter("pos_participant_control", "situational")
            return
                <value>{$x}</value>
        }</field>
    <field
...
declare function local:dynSituational($fs as item()*, $filters as xs:string+) as item()* {
    ...
    let $filtered := if ($filters = 'none') then
        $fs[ft:query-field(., "modalType:situational")][not(tei:f[@name eq 'subtype'])]
    else
        $fs[ft:query-field(., $query)]
    return
        $filtered
...
declare function local:dynamic($fs as item()*) as item()* {
    ...
    let $pos_fs_filtered1 := if (count($pos_parts) gt 0) then
        local:dynParticipants($dyn_fs[ft:query-field(., 'modalMeaning:possibility')], $pos_parts)
    else
        ()
    let $pos_fs_filtered2 := if (count($pos_sit_types) gt 0) then
        local:dynSituational($dyn_fs[ft:query-field(., 'modalMeaning:possibility')], $pos_sit_types)
    else
        ()
    let $nec_fs_filtered1 := if (count($nec_parts) gt 0) then
        local:dynParticipants($dyn_fs[ft:query-field(., 'modalMeaning:necessity')], $nec_parts)
    else
        ()
    let $nec_fs_filtered2 := if (count($nec_sit_types) gt 0) then
        local:dynSituational($dyn_fs[ft:query-field(., 'modalMeaning:necessity')], $nec_sit_types)
    else
        ()
...
declare function local:dynamic($fs as item()*) as item()* {
    ...
    let $dyn_fs_filtered := if (exists($pos_fs_filtered) or exists($nec_fs_filtered)) then $pos_fs_filtered |
    $nec_fs_filtered else $dyn_fs
    for $dyn in $dyn_fs_filtered
...
declare function local:deontic($fs as item()*) as item()* {
    ...
    let $other1 := if ($deo/tei:f[@name eq 'context']) then
        $deo/tei:f[@name eq 'context']/tei:symbol/@value/string()
    else
        ()
    let $other2 := if ($deo/tei:f[@name eq 'source']) then
